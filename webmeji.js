...
window.activeWebmejis = []; // Create global list
  ...
  Promise.all(configs.map(preloadImages))
    .then(() => {
      window.SPAWNING.forEach(({ id, config }) => {
        ...
        if (cfg) {
          const creature = new Creature(id, cfg);
          window.activeWebmejis.push(creature); // Store globally
        }
      });
    });
});
...
class Creature {
  constructor(containerId, spriteConfig) {
    ...
    // Inside the constructor, right after this.container.appendChild(this.img);
  ...
  setTimeout(() => {
    if (window.activeWebmejis) {
      window.activeWebmejis.forEach(meji => {
        // meji.element 是桌宠的 DOM 容器
        meji.element.style.pointerEvents = 'auto'; // 确保容器可以接收点击
        
        meji.element.addEventListener('click', () => {
          meji.say("你点我干嘛？");
          
          // 可选：让它说话 3 秒后自动消失
          setTimeout(() => meji.shush(), 3000);
        });
      });
    }
  }, 1000); 
